...
param_file = "fixedmodel"
###################################################################################################################
for i in range(0, len(datasets)):
    # variables to change
    nrIterations = [1, 3, 5, 7, 9, 11]  #lines
    minlifts = [1.1, 1.25, 1.5, 1.6, 1.75] #x-axis
    ...
    if not use_file:
        ...
        for nrIter in nrIterations:
            jumping=False
            yvalues = []
            for minlift in minlifts:
                if jumping:
                    break
                with open(f"./files/bic_var/{param_file}.json", "r") as f:
                    d = json.load(f)
                    if not recalculate_all and f"{nrIter}_{minlift}" in d.keys():
                        print(f"Iterations: {nrIter} | MinLift: {minlift} \tAlready calculated, continuing")
                        yvalues.append(d[f"{nrIter}_{minlift}"])
                        continue

                print(f"Iterations: {nrIter} | MinLift: {minlift} \tNot continued, calculating...")
                ...
                for fold, (train_index, test_index) in enumerate(skf.split(X, y)):
                    ...
                    jar_command = f"java -jar bic.jar {nrbics} {nrIter} {minlift} {1} {nrlabels}"
                    res = os.system(dir_command + ";" + jar_command)
                    if nrIter==3 and minlift==1.15 and fold==1:
                        res = -1
                    if nrIter==4 and minlift>=1.12:
                        res = -1
                    if res != 0:
                        print(f"\n\nError at iterations: {nrIter}, lift: {minlift}, fold: {fold}\n\n")
                        jumping=True
                        break
                    ...
                    jar_command = f"java -jar bic.jar {nrbics} {nrIter} {minlift} {2} {nrlabels}"
                    res = os.system(dir_command + ";" + jar_command)
                ...
                if not jumping:
                    ...
                    with open(f"./files/bic_var/{param_file}.json", "r+") as f:
                        d = json.load(f)
                        d[f"{nrIter}_{minlift}"] = statistics.mean(temp_yvalues)
                        f.seek(0)
                        json.dump(d, f)
                        f.truncate()

            values[nrIter] = yvalues
    ...
    for jk in values:
        dif = len(minlifts) - len(values[jk])
        if dif > 0:
            values[jk] = values[jk] + ([0] * dif)
    ds.multiple_line_chart(minlifts, values, ax=axs[0, 0], title='AUC of Decision Tree per number of iterations and minimum lift',
                           xlabel='Minimum lift', ylabel='AUC', percentage=True)
    plt.tight_layout()
    #plt.show()
