...
for i in range(0, len(datasets)):
    # variables to change
    nrLabels = [ 3, 4, 5, 6]  #lines
    bicsBeforeMerging = [50, 75, 100, 125, 140] #x-axis


    dt = DecisionTreeClassifier  # lines
    ...
    if not use_file:
        ...
        values = {}
        for nrlab in nrLabels:
            yvalues = []
            for bics in bicsBeforeMerging:

                with open(f"./files/bic_var/{param_file}.json", "r") as f:
                    d = json.load(f)
                    if not recalculate_all and f"{nrlab}_{bics}" in d.keys():
                        print(f"Labels: {nrlab} | BicsBeforeMerging: {bics} \tAlready calculated, continuing")
                        yvalues.append(d[f"{nrlab}_{bics}"])
                        continue

                print(f"Labels: {nrlab} | BicsBeforeMerging: {bics} \tNot continued, calculating...")
                ...
                for fold, (train_index, test_index) in enumerate(skf.split(X, y)):
                    ...
                    jar_command = f"java -jar bic.jar {bics} {nrIter} {minlift} {1} {nrlab}"
                    print("\n" + dir_command, " ; ", jar_command)
                    res = os.system(dir_command + ";" + jar_command)
                    print(f"Result of java: {res}")
                    ...
                    jar_command = f"java -jar bic.jar {bics} {nrIter} {minlift} {2} {nrlab}"
                    print("\n" + dir_command, " ; ", jar_command)
                    res = os.system(dir_command + ";" + jar_command)
                    print(f"Result of java: {res}")
                    ...
                    params = optimize.optimize_model(model, df_train, target, metric[0], balancing, scaling, feature_scale,
                                                    MAX_EVALS, k=5, early_stop=early, rs=rs, verbose=optimize_verbose)
                    # params = {}  # DEBUG
                    # print(params)
                ...
                with open(f"./files/bic_var/{param_file}.json", "r+") as f:
                    d = json.load(f)
                    d[f"{nrlab}_{bics}"] = statistics.mean(temp_yvalues)
                    f.seek(0)
                    json.dump(d, f)
                    f.truncate()

            values[nrlab] = yvalues
    ...
    ds.multiple_line_chart(bicsBeforeMerging, values, ax=axs[0, 0], title='AUC of Decision Tree per number of labels and number of bics before merging',
                           xlabel='Bics before merging', ylabel='AUC', percentage=True)
    plt.tight_layout()
    #plt.show()
