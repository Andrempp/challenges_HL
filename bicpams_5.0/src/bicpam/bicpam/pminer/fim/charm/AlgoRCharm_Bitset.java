...
import bicpam.pminer.fim.eclat.TriangularMatrix;
 
/**
 * This is a new implementation of the CHARM algorithm (2014) that relies on bitsets to implement tidsets.
 * Charm was proposed by ZAKI (2001) <br/><br/>
 ...
 */
public class AlgoRCharm_Bitset {
	...
	public AlgoRCharm_Bitset(List<List<Integer>> _dataset) {
		dataset = _dataset;
	}
	...
	public Itemsets runAlgorithm(double minsuppAbsolute, int minCols) throws IOException {
		System.out.print("Support:"+minsuppAbsolute+"|");
		this.hash = new HashTable(100);
		minColumns = minCols;
		return runAlgorithm(minsuppAbsolute,false,10000);
	}
	 ...
	 */
	public Itemsets runAlgorithm(double minsup, boolean useTriangularMatrixOptimization, int hashTableSize) throws IOException {
		writer = null;
		...
		this.minsupRelative = (int) Math.ceil(minsup * dataset.size());
		...
		int maxItemId = 0;
		maxItemId = calculateSupportSingleItems(mapItemTIDS, maxItemId);
		...
		if (useTriangularMatrixOptimization) {
			...
			for(List<Integer> row : dataset){
				for(int j=0, l2=row.size(); j<l2; j++)
					for(int k=j+1; k<l2; k++) matrix.incrementCount(row.get(j),row.get(k));
			}
		}
		...
		for(Entry<Integer, BitSetSupport> entry : mapItemTIDS.entrySet()) {
			...
			if(support >= minsupRelative) {
				...
				if(minColumns<=1){
					closedItemsets.addItemset(new Itemset(new int[]{item}, tidset.bitset, support), 1);
				}
			}
		}
...
loopJ:		for(int j=i+1; j < frequentItems.size(); j++) {
	...
	private int calculateSupportSingleItems(final Map<Integer, BitSetSupport> mapItemTIDS, int maxItemId) {
	...
	private void save(int[] prefix, int[] suffix, BitSetSupport tidset) throws IOException {
		...
		//System.out.println("Save:"+BicPrinting.plot(prefixSuffix)+"|"+tidset.bitset);
		if(prefixSuffix.length<minColumns) return;

		// Sort the resulting itemset
		Arrays.sort(prefixSuffix);
		
		// Create an instance of "Itemset" for that itemset to put in hash table
		bicpam.pminer.fim.charm.count.Itemset itemset = new bicpam.pminer.fim.charm.count.Itemset(prefixSuffix);
