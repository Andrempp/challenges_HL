...
public class F2KTree {
	...
	/** Create list of items in the header table */
	public void createHeaderList(final Map<Integer, Integer> mapSupport, final int nrLabels) {
		headerList =  new ArrayList<Integer>(mapItemNodes.keySet());
		Collections.sort(headerList, new Comparator<Integer>(){
			public int compare(Integer id1, Integer id2){
				int compare = id1%nrLabels - id2%nrLabels; 
				if(compare==0) return id1-id2;
				else return compare;
			}
		});
	}
