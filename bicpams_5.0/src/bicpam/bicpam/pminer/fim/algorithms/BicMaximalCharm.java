...
import bicpam.pminer.fim.apriori.AlgoCharmMFI_Bitset;
 ...
 * @version 1.0
 */
public class BicMaximalCharm extends AlgoCharmMFI_Bitset implements BicPM {

	public BicMaximalCharm(List<List<Integer>> dataset) {
		super(dataset);
	}
	...
	public Biclusters run(int minColumns, int minRows, int maxColumns, int maxRows, int nrLabels) throws Exception {
	  Itemsets itemsets = super.runAlgorithm((double)minRows/(double)dataset.size());
	  //System.out.println("#"+itemsets.getItemsetsCount()+"\n"+itemsets.toString());
	  //Biclusters freqItemsets = BicFIMUtils.toBicItemsets(itemsets);
