...
public class PseudoSequence {
	...
	public PseudoSequence(PseudoSequence sequence, int indexItemset, int indexItem){
		...
		// Last itemset and item (par d�faut c'est le dernier item et itemset de la s�quence)
	...
	public  List<PseudoSequencePair> getAllInstancesOfPrefix(Sequence prefix, int i){
		...
		// On coupe les instances trouv�s selon la taille maximale du pr�fix cherch�.
		List<PseudoSequencePair> allPairs = new ArrayList<PseudoSequencePair>();
		for(List<Position> listPositions : listInstances){
			PseudoSequence newSequence = new PseudoSequence(this, this.firstItemset, this.firstItem, 
		...
		for(int i=decalageItemset; i< size(); i++){
			...
			for(int j=0; j < getSizeOfItemsetAt(i); j++){
				...
				if(id == iDCourant){// l'item match
					...
					}else if(indexItem+1 >= prefix.get(indexItemset).size()){ // si on a trouv� tout l'itemset
						
						List<Position> newList = new ArrayList<Position>(listPositionsTotal);
						newList.addAll(listPositions);
						
						if(indexItemset+1 < prefix.size()){ // NOUVEAU POUR R�GLER UNE EXCEPTION 2008-09  C'EST CORRECT ???
	 ...
	 * @param i : le ime element du prefixe.
	 * @return
	 */
	public Position getIthLastInLastApearanceWithRespectToPrefix(Sequence prefix, int i, boolean withTimeStamps){
		...
		if(withTimeStamps == false){
			...
			int iditem = prefix.getIthItem(i);
	...
	public PseudoSequence trimBeginingAndEnd(Position positionStart, Position positionEnd){
		...
		return new PseudoSequence(this, itemsetStart, itemStart, itemsetEnd, itemEnd);
	}
	 ...
	 * @param i : the ime element of the prefix.
	 * @return
	 */
