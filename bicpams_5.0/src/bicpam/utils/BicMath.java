...
public class BicMath {
	...
	public static double maxindex(double[] vector) {
		double max = Double.NEGATIVE_INFINITY, index = -1;
	...
	public static double count(List<List<Integer>> data) {
		double count = 0;
		for(Collection<Integer> entry : data) count+=entry.size();
		return count;
	}
	...
	public static double varianceL(List<List<Double>> matrix) {
	...
	public static int[] generateVector(int i, int j) {
		int[] result = new int[j-i+1];
		for(int k=0; i<=j; i++, k++) result[k]=i;
		return result;
	}
}
